...
*/
class PartsReceiptController extends BaseController
{
    use Curd;
    ...
    */
        ...
        if ($model->load(Yii::$app->request->post())) {
            try{
                ...
                $log = [
                        ...
                        'log_type' => LogTypeEnum::ARTIFICIAL,
                        'log_module' => '单据审核',
                        'log_msg' => "配件收货单审核,审核状态：".AuditStatusEnum::getValue($model->audit_status).",审核备注：".$model->audit_remark
                ];
                \Yii::$app->purchaseService->receiptLog->createReceiptLog($log);
    ...
    public function actionView()
    {
        ...
        $returnUrl = Yii::$app->request->get('returnUrl',Url::to(['parts-receipt/index']));
        if(!$id){
    ...
    public function actionDelete($id)
    {
        ...
        try{
            ...
            $goods = PurchasePartsReceiptGoods::find()->select('purchase_detail_id')->where(['receipt_id'=>$id])->all();
            $ids = ArrayHelper::getColumn($goods, 'purchase_detail_id');
            $res = PurchasePartsGoods::updateAll(['is_receipt'=>ConfirmEnum::NO], ['id'=>$ids]);
            if(false === $res){
                throw new \Exception("更新采购单明细商品状态失败");
            }
            $res = PurchasePartsReceiptGoods::deleteAll(['receipt_id'=>$id]);
            if(false === $res){
    ...
    public function actionExport($ids=null){
        ...
        $list = PurchaseReceipt::find()->alias('pr')
            ->leftJoin(PurchasePartsReceiptGoods::tableName().' prg','pr.id = prg.receipt_id')
    ...
    public function actionPrint()
    {
        ...
        $returnUrl = Yii::$app->request->get('returnUrl',Url::to(['parts-receipt/index']));
        $model = $this->findModel($id);
        $goodsModel = new PurchasePartsReceiptGoods();
