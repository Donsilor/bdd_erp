...
//            }],
    ...
    {
        $bill = WarehouseStoneBill::find()->alias('wb')
            ->leftJoin(Supplier::tableName(). ' sup', 'sup.id=wb.supplier_id')
            ->where(['wb.id'=>$id])->select(['wb.*','sup.name as supplier_name'])->one();

        $query = WarehouseStoneBillGoods::find()->alias('wg')
           ->where(['wg.bill_id' => $id]);
...
//        print_r($lists);die;
        $total = [
            ...
            'delivery_no' => $bill->delivery_no,
            'supplier_name' => $bill->supplier_name,
            'audit_time' => Yii::$app->formatter->asDate($bill->audit_time,'Y年m月d日'),
            'cost_price' => 0,
        ];
