...
use addons\Warehouse\common\forms\WarehouseBillWfForm;
...
class BillWfController extends BaseController
{
    ...
    public $billType = BillTypeEnum::BILL_TYPE_WF;
        ...
        if ($model->load(\Yii::$app->request->post())) {
            try{
                ...
                if($model->isNewRecord){
                    ...
                    $log_msg = "创建维修调拨单{$model->bill_no}，入库仓库为{$model->toWarehouse->name}";
                }else{
                    $log_msg = "修改维修调拨单{$model->bill_no}，入库仓库为{$model->toWarehouse->name}";
                }
    ...
    public function actionPrint()
    {
        ...
        $model = $this->findModel($id);
        $goodsModel = new WarehouseBillGoods();
