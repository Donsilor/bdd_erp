...
{
    use Curd;
    public $modelClass = Follower::class;
    /**
    ...
    public function actionIndex()
    {
        ...
        if (!empty($updated_at)) {
            $dataProvider->query->andFilterWhere(['>=',Follower::tableName().'.updated_at', strtotime(explode('/', $updated_at)[0])]);//起始时间
            $dataProvider->query->andFilterWhere(['<',Follower::tableName().'.updated_at', (strtotime(explode('/', $updated_at)[1]) + 86400)] );//结束时间
        }

        $dataProvider->query->andWhere(['>',Follower::tableName().'.status',-1]);
