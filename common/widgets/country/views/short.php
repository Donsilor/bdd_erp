...
<div class="row">
    ...
    <?php if ($level >= 2){ ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $provinceName)->dropDownList(Yii::$app->services->country->getProvinceMapByPid($model->$countryName, 2), [
                    'prompt' => '-- 请选择省份 --',
    ...
    <?php if ($level >= 3){ ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $cityName)->dropDownList(Yii::$app->services->country->getProvinceMapByPid($model->$provinceName, 3), [
                'prompt' => '-- 请选择城市 --',
            ]) ?>
        </div>
    <?php }?>
